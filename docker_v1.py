...
if sensorHubInput[STATUS_REG] & 0x01 :
    ...
    print("Current off-chip sensor temperature = %d Celsius" % sensorHubInput[TEMP_REG])
...
if sensorHubInput[STATUS_REG] & 0x04 :
    ...
    #Units = Lux
    print("High BITS: %d" % (sensorHubInput[LIGHT_REG_H] << 8))
    print("Low BITS: %d" % sensorHubInput[LIGHT_REG_L])
    brightness = (sensorHubInput[LIGHT_REG_H] << 8 | sensorHubInput[LIGHT_REG_L])
    print("Current onboard sensor brightness = %d Lux" % (sensorHubInput[LIGHT_REG_H] << 8 | sensorHubInput[LIGHT_REG_L]))
...
print("Current onboard sensor temperature = %d Celsius" % sensorHubInput[ON_BOARD_TEMP_REG])
print("Current onboard sensor humidity = %d %%" % sensorHubInput[ON_BOARD_HUMIDITY_REG])
...
if sensorHubInput[BMP280_STATUS] == 0 :
    ...
    print("Current barometer temperature = %d Celsius" % sensorHubInput[BMP280_TEMP_REG])
    print("Current barometer pressure = %d pascal" % (sensorHubInput[BMP280_PRESSURE_REG_L] | sensorHubInput[BMP280_PRESSURE_REG_M] << 8 | sensorHubInput[BMP280_PRESSURE_REG_H] << 16))
else :
